...
Server side only requests the data that is needed and search engines are optimized for web pages that come from the server, but every request results in a full page refresh.
Client side has smooth transitions between views and changing between those views tends to be faster, but the whole website must be loaded on the first request which may result in a longer initial load. Also since the entire website is loaded you may download data for pages you may not visit.
